// src/webs/llm/svc-review.js
//
// <svc-review> — form output cuối (title/description/content/pics qua <svc-editor>) + nút Tạo
// lại/Lưu. Cloned from src/webs/division/svc-review.js (domain-isolation clone) — presentational,
// KHÔNG tự biết D1/collection nào, chỉ nhận `doc`/`txt` qua prop và phát field-change/regenerate/
// save cho cha tự xử lý persist. Not currently used by svc-talk.js's own flow (same as the
// division original — svc-review.js's actual caller there is admin-base.js's review phase, which
// has no equivalent in this domain); included for parity/future reuse.
...
// Tag name 'svc-review', NOT 'svc-review' — avoids a global Custom Elements registry collision
// with division/svc-review.js (see svc-progress.js's header for the full rationale).
