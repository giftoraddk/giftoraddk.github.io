...
> thật này, map trực tiếp vào `hook/SCHEMA.rst` + `hook/ARCHITECT.rst` — theo cùng nguyên tắc đã áp
> dụng ở `hook/new_feature.md`: **không tạo hạ tầng mới khi cái đã có đủ dùng**.
...
ghi qua `createService('products')` / `createService('posts')` (`hook/CRUD.rst`). Mục tiêu:
...
- Codebase **đã có tiền lệ rõ ràng** cho việc né cron/worker: `hook/new_feature.md` §2.2 tự nhận
...
đã được repo chấp nhận cho `meta.views`/`meta.likes` bump (`hook/SCHEMA.rst`) và cache
`users.meta.xuBalance` (`hook/new_feature.md` §3.2): tắt tab giữa chừng chỉ mất phần **chưa xử
...
`products`/`posts` **đã là 2 collection riêng** (đúng phát hiện của `hook/new_feature.md` §0: mỗi
loại dữ liệu trong `hook/SCHEMA.rst` map ra 1 collection Firestore riêng, `mode` chỉ còn giá trị
...
Không cần sửa `hook/SCHEMA.rst` bảng chính — chỉ cần thêm 1 đoạn mô tả `meta.geo` vào mục "`meta`
...
Theo đúng khung leaf-domain (`hook/ARCHITECT.rst` §Micro Service Pattern), tham chiếu gần nhất là
...
`hook/new_feature.md` §7: đây là logic có điều kiện thật (trộn 2 collection, tính score, điều
...
`hook/ARCHITECT.rst` §Comment Convention):
...
`hook/new_feature.md` §8 dùng cho `svc-talent-profile`:
...
Thêm `'geo'` vào `TABLES_STD` (`src/webs/auth/svc-roles.js:15-18`, đúng cách `hook/new_feature.md`
...
1. **Doc**: thêm đoạn `meta.geo` shape vào `hook/SCHEMA.rst` (mục "`meta` shape theo `mode`"),
...
9. **Kiểm tra field FAQ trên `posts`**: schema hiện tại (`src/services/schemas/admin/posts.js`)
   ...
   `repeater`, đúng field type đã có sẵn cho `web-table`/`svc-assist`, xem `hook/SCHEMA.rst`) hay
...
`hook/CRUD.rst`), trigger bằng GitHub Actions scheduled workflow gọi route đó với 1 secret header.
...
5. **`geo` là pseudo-table trong RBAC, không phải collection thật** (§8) — nếu sau này GEO cần lưu
   ...
   `records mode: 'geo_run'` giống mẫu `wallet_txn` append-only trong `hook/new_feature.md` §1.6 —
...
- `src/pages/shop/index.astro`/`gift/index.astro` **không** SSG — chỉ render `<web-board>`, dữ
  ...
  `hook/ARCHITECT.rst`). Khái niệm "revalidate HTML" không áp dụng ở đây — cái cần "làm mới" là
...
(`hook/SERVICES.rst`). Không có gì để implement ở tầng này.
...
`loadData()` dùng — xem `hook/CRUD.rst`) — lần load TIẾP THEO (reload trang, tab mới) lấy dữ liệu
mới ngay, không cần chờ hết TTL 5 phút. **Không** đẩy dữ liệu mới vào tab đang mở sẵn (section đã
có data trong RAM của conductor không tự re-fetch, đúng thiết kế `hook/SERVICES.rst`) — đây là hạn
...
`hook/SCHEMA.rst`), không phải field bịa như `_cardProductNeat.js` (`meta.oldPrice`/`meta.badge`).
