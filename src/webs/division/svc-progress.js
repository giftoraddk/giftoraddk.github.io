// src/webs/division/svc-progress.js
//
// <svc-progress> — cây macro/sub-step (tiến trình + kết quả từng phần), DÙNG CHUNG cho
// svc-marketing.js (chạy pipeline thật, phase running/review) VÀ svc-talk.js (popup "Xem chi
// tiết" của 1 job chat). Thay thế hẳn tools/progress-view.js (đã xoá) lẫn phần progress UI cũ nằm
// trong svc-marketing.js — xem hook/superpowers/specs/2026-09-05-division-talk-design.md.
//
// Presentational — KHÔNG tự biết Firestore/collection nào, KHÔNG tự gọi AI. Nhận `division`+`doc`
// (doc dạng job-like: {fields, status, error, topic, colors, language}) qua prop, phát CustomEvent
// khi user tương tác — cha (svc-marketing/svc-talk) tự nghe rồi tự quyết định persist/chạy lại AI
// ở đâu (mỗi cha có 1 Firestore collection/doc shape khác nhau).
//
...
// === 'skip' — xem svc-talk.js/admin-base.js's _dhImageChoice) — không thì step ảnh bị bỏ qua sẽ
// hiện 'pending' mãi mãi dù job đã 'done' thật.
...
// PENDING đầu tiên khi doc chưa done/lỗi. Caller (svc-marketing) có tracking thật (reqId-scoped) thì
// truyền thẳng `runningStep`/`failedStep` qua prop, ghi đè phần suy luận này. `awaiting_review` —
// job đang DỪNG chờ sếp quyết định (xem _comPausedStep) — không có gì đang "chạy" thật lúc này.
...
export class SvcProgress extends LitElement {
    ...
    static properties = {
        ...
        approvable:  { type: Boolean }, // hiện nút "Duyệt" khi doc đã xong & chưa duyệt, phát `approve` — CHỈ svc-talk.js bật cờ này (job -> tạo record mới); admin-base.js (svc-marketing/production) không dùng, Lưu đã có riêng qua <svc-review>
        regeneratingImage: { type: Boolean }, // cha set true trong lúc chờ AI tạo lại ảnh (loading nút)
        busy: { type: Boolean }, // cha set true khi job/doc này đang có 1 hành động (retry/resume/regenerate-call/image-choice/approve) chưa xong — disable mọi nút hành động khác, tránh bấm chồng chạy trùng
        ...
        // Map<"stepId-callKey", string>, thuần local UI state (KHÔNG persist, cha không cần biết
        // cho tới lúc bấm "Tạo lại" mới đọc qua CustomEvent's detail.instruction).
    ...
    get _comPausedStep() {
        ...
        return this.division.meta.steps.find(s => s.id === this.doc.pausedStepId) || null
    }

    // Sếp đã chọn "Bỏ qua" AI ở step ảnh nhưng CHƯA thật sự có ảnh — `stepDone()` vẫn coi step này
    // "xong" nên job KHÔNG chờ (đúng ý ban đầu — không block các division khác), NHƯNG pipeline vì
    // vậy có thể đã tiến sang step SAU đó, khiến `_comActiveMacroStep` tự nhảy theo tiến độ mới,
    // "giấu" luôn tab step ảnh (chứa 3 nút mở AI ngoài + ô upload cho sếp tự làm — xem
    // `_rfImageSubContent`'s `manualOk`) sau nhiều cú click mới xem lại được. Ghim tab ở ĐÚNG step
    // ảnh này (như 1 dạng "chờ xử lý" ảo) cho tới khi có `pics` thật, để 3 nút + input LUÔN hiện
    // sẵn ngay khi mở popup — không cần bấm lùi lại step cũ.
    ...
    // bị web-steps.js tự đóng/chặn click mở (xem web-steps.js's `_isOpen`/`_toggleVerticalOpen`).
    // Riêng 'generate'/'upload' của step ảnh khi sếp đã chọn "Bỏ qua" (chưa có `pics` thật) PHẢI
    // khác 'pending' — nếu không, 3 nút mở AI ngoài + ô upload (đã đủ điều kiện hiện qua
    // `_rfImageSubContent`'s `manualOk`) vẫn bị web-steps.js giấu kín, sếp không cách nào mở ra được.
    _subStatus(step, key) {
        const k = `${step.id}-${key}`
    ...
    // xem svc-talk.js/admin-base.js's _dhImageChoice.
    ...
    _rfImageSubContent(step, key) {
        ...
        // "Bỏ qua" — xem svc-talk.js/admin-base.js's _dhImageChoice/_dfImageChoice). Riêng trường
        // hợp "Bỏ qua", job có thể CHƯA 'done' (image không phải step cuối ở division khác) nhưng
        // sếp vẫn cần 3 nút mở trang ngoài NGAY để tự làm ảnh — không thể chờ tới lúc cả job xong.
