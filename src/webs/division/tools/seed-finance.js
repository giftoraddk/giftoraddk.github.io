// src/webs/division/tools/seed-finance.js
//
// Chuyên môn phòng "Tài chính - Kế toán" bóc tách thành JSON config đọc bởi tools/engine.js — nén
// từ khung suy luận 41 bước trong hook/finance_accounting.md (DATA->CHECK->CLASSIFY->RECONCILE->
// ANALYZE->ASSESS->RISK->RECOMMEND->DECIDE, §5) xuống 1 step thực dụng, cùng tinh thần với
// tools/seed-production.js's nén 28-step framework xuống 4 step.
//
...
// webs/report/tools/finance-data.js) rồi patch vào `fields` TRƯỚC khi chạy step này — GIỐNG hệt
// cách `productContext` được bơm cho division `requiresProduct` (xem svc-talk.js's _runJob).
//
...
export const FINANCE_DIVISION_SEED = {
    ...
    ai: '',
    meta: {
        // Collection lưu tiến trình — chưa có popup admin riêng nào cho finance (khác marketing/
        // production đều có /admin's popup AI Content), nên để trống — svc-talk.js's _dfApprove tự
        // bỏ qua bước mirror processTable nếu rỗng (xem đó, "if (division.meta.processTable)").
        processTable: '',
        steps: [
            {
        ...
        // `finance_reports` — lưu lại làm DECISION MEMO có thể tra cứu sau (hook/finance_accounting.md
        // §37) — collection MỚI, chưa có trang /admin CRUD riêng (ngoài phạm vi yêu cầu hiện tại).
