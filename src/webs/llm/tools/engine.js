// src/webs/llm/tools/engine.js
//
// Generic pipeline engine shared by every division — reads a division's `meta.steps` JSON config
// instead of hardcoded per-department prompt builders. A `step` of `type: 'text'` is fully
// data-driven here; a step of `type: 'image'` is the one non-generalizable exception, handled by
// tools/image.js.
//
// Cloned from src/webs/division/tools/engine.js (domain-isolation clone, see webs/llm/svc-talk.js's
// header) — logic unchanged, only this file's own callers differ (svc-talk.js in THIS domain,
// there is no svc-marketing.js equivalent here).
//
// Caller (svc-talk.js) owns the actual doc shape / D1 persistence / UI running-vs-done tracking —
// this file only builds prompts, orders calls, and calls the AI.
import { generateText, DEFAULT_CHAIN } from '@/services/tensor.js'
 ...
 * `onAttempt(attempt, maxAttempts)` (tuỳ chọn) gọi ngay đầu mỗi lần thử — cùng convention với
 * `runSub`/`onPatch` của `runTextStep` bên dưới — để UI hiển thị tiến độ (vd svc-aide.js's "đang
 * thử lại...").
 */
export async function generateJsonWithRetry(ai, { system, user, maxTokens, temperature, languageName, errMessage, onAttempt }) {
    let lastErr
    for (let attempt = 1; attempt <= 3; attempt++) {
        ...
        try {
            ...
            if (_hasForeignScript(obj, languageName)) {
                console.error('[llm/engine] response lẫn ký tự ngôn ngữ khác:', obj)
                throw new Error(errMessage)
            }
            return obj
        } catch (err) {
            lastErr = err
            console.warn(`[llm/engine] JSON generation attempt ${attempt}/3 failed:`, err.message)
            // _parseJsonObject lỗi = request đã THÀNH CÔNG (có text trả về), chỉ là không phải JSON
            // hợp lệ — model/key thật giờ do Worker chọn mỗi request (xem worker/packages/llm-worker/src/ai.ts), phía
            // client không còn biết (key, model) nào vừa chạy để demote nữa; retry kế tiếp có thể
            // trúng lại đúng model vừa trả dữ liệu hỏng — đánh đổi chấp nhận được so với việc giữ
            // real key phía client chỉ để phục vụ demote.
        }
    }
    throw lastErr
}
...
// Chuỗi "chain" cuối cùng truyền vào generateJsonWithRetry — `@default` là chain chuẩn của Worker
// (fallback GROQ/OPER/NVID theo latency, xem worker/packages/llm-worker/src/ai.ts's resolveChain()), rồi tới AI riêng
// của division/entity (`entityAi`, override qua /admin — vẫn 1 raw key thật, CHƯA được bảo mật qua
// Worker) — dùng chung bởi svc-talk.js VÀ svc-aide.js trong domain này. Real keys không còn xuất
// hiện ở client nữa ngoại trừ đúng trường hợp entityAi này.
...
export function orderCallWaves(calls) {
    ...
    while (remaining.length) {
        ...
        if (!wave.length) throw new Error('[llm/engine] circular dependsOn in step config')
 ...
 * stamp status/error (mỗi consumer có status vocab hơi khác nhau).
 *
 ...
 *   running/done cho UI (key = call.key) — xem svc-talk.js's _runSub
...
export async function regenerateCall(step, callKey, instruction, ctx) {
    ...
    if (!call) throw new Error(`[llm/engine] call not found: ${callKey}`)
    const { topic, languageName, ai, fields, errMessage, runSub, onPatch } = ctx
    return runSub(call.key, async () => {
