// src/webs/division/svc-production.js
//
...
//
// Chuyên môn đọc từ Firestore collection `divisions` (doc id 'production'; seed 1 lần từ
// tools/seed-production.js nếu chưa tồn tại) — cùng division svc-talk.js dùng làm tiền đề "tạo sản
// phẩm trước" cho các phòng ban khác (xem svc-talk.js's `requiresProduct`).
//
// Toàn bộ orchestration (data/pipeline/Firestore) sống ở tools/admin-base.js's DivisionAdminBase —
// dùng chung với svc-marketing.js (mirror y hệt, khác division/table/seed/text). File này chỉ khai
// default props + hằng số riêng của phòng Sản phẩm & Vận hành.
