// src/webs/division/svc-finance-report.js
//
// <svc-finance-report> — "Dashboard Phân Tích Doanh Thu Sản Phẩm" cho /admin/report, dựng theo
// hook/product-revenue-dashboard-spec.md (KPI + Revenue/Profit theo tháng + Top sản phẩm bán chạy +
// Doanh thu theo sản phẩm + bảng chi tiết). Đọc thẳng `invoices` (Firestore, cùng collection với
// domain `pay` — xem hook/PAY.rst §3.3) + `products` (giá vốn/danh mục) — KHÔNG có bảng
// orders/orderItems/categories riêng nào, nên "join" (spec §4 Bước 2) map sang: 1 invoice ~ 1
// "order", `parseInvoiceItems(invoice.items)` ~ "orderItems", category lấy từ tag đầu tiên của
// product khớp TÊN. Business logic thuần tách hẳn ra tools/revenue-helper.js (đọc file đó cho GIỚI
// HẠN DỮ LIỆU THẬT — đặc biệt `discount` luôn 0 và `category` suy ra từ tag, không phải bảng riêng).
//
// KHÁC tools/finance-helper.js (dùng bởi svc-talk.js's division 'finance' — dòng tiền/công nợ/rủi
// ro cấp INVOICE) — file đó giữ nguyên, không đụng vào. Dashboard này tính lại ở cấp SẢN PHẨM.
    ...
    async _dcLoad() {
        ...
        try {
            const [invoices, products] = await Promise.all([
                createService('invoices', '', 'invoices').findAll(),
