// src/webs/llm/tools/finance-helper.js
//
// Tính toán THUẦN (không I/O, không Lit) cho ngữ cảnh tài chính thật của division 'finance' trong
// domain này (xem svc-talk.js's _comFinanceContext). Cloned from
// src/webs/division/tools/finance-helper.js (chính file đó lại là bản sao gần như nguyên vẹn của
// src/webs/report/tools/finance-data.js — xem comment ở đó) — domain-isolation clone, logic
// unchanged. Tái dùng `parseInvoiceItems`/`isPendingPayment`/`needsRefund` từ domain `pay` — dùng
// CHUNG hàm THUẦN cross-domain, không phải phụ thuộc ngược vào state/UI của pay (giống cách
// `webs/report`/`webs/division` đã dùng, xem hook/PAY.rst §1 điểm 4).
//
