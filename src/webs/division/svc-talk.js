// src/webs/division/svc-talk.js
//
// <svc-talk> — 1 nhóm chat vui vẻ, tự nhiên giữa sếp và các "phòng ban AI". Widget độc lập, tự
// vẽ UI chat riêng (KHÔNG dùng <svc-chat> — svc-chat.js giữ nguyên hiện trạng cho domain chat
// gốc của nó). Gắn 1 lần trong LayoutAdmin.astro (giống <svc-roles>), tự ẩn nếu chưa có division
// nào `active`.
//
...
// lúc mở. Không tự tay làm lại cơ chế này (đã thử bằng tay trước đó, kém hơn hẳn).
//
...
// có, tự rút ra topic rồi chạy pipeline chuyên môn thật (tools/engine.js + tools/image.js, y hệt
// svc-marketing.js dùng) ngay sau lượt chat, rồi báo kết quả kèm nút Duyệt/Tạo lại.
//
// 1 collection Firestore DUY NHẤT (`talks`) — mỗi doc là 1 "bubble" trong nhóm chat, kind:'chat'
// (câu nói thường) hoặc kind:'job' (tiến trình + kết quả pipeline, đọc/ghi bag `fields` phẳng
// giống svc-marketing.js). Xem hook/superpowers/specs/2026-09-05-division-talk-design.md (đã lỗi
// thời phần UI — giữ lại phần data-model chuyên môn/engine).
...
import '@/webs/division/svc-progress.js'
...
import { syncMindFromOutputTable } from './tools/mind-sync.js'
...
import { computeOverview, computeReceivablesPayables, assessRisk } from '@/webs/division/tools/finance-helper.js'
import styles from './styles/svc-talk.css?inline'

// Seed mặc định — svc-marketing.js cũng tự seed 'marketing' khi mở popup AI Content lần đầu, nhưng
// svc-talk.js có thể load TRƯỚC đó (vd admin chưa từng mở popup marketing) nên tự đảm bảo luôn ở
// đây cho chắc (_dcEnsureSeeds). 'production'/'finance' không có popup admin riêng nên LUÔN seed
// qua đây.
...
export class SvcTalk extends LitElement {
    ...
    async connectedCallback() {
        ...
        try {
            const rows = await createService('divisions').findAll()
            this._divisions = await this._dcEnsureSeeds(rows.filter(d => d.status === 'active'))
        } catch (err) {
            console.error('[svc-talk] failed to load divisions:', err.message)
            this._divisions = []
        }
        try {
            this._log = await createService('talks').findAll({ sortBy: 'created_at', order: 'asc' })
        } catch (err) {
            console.error('[svc-talk] failed to load talks:', err.message)
            this._log = []
        }
    }
    ...
    async _dcEnsureSeeds(activeRows) {
        ...
        for (const [id, seed] of Object.entries(DEFAULT_SEEDS)) {
            ...
            try {
                ...
                console.error(`[svc-talk] failed to seed division '${id}':`, err.message)
            }
        }
        return [...byId.values()]
    }
    ...
    _enqueue(fn) {
        this._queue = this._queue.then(fn).catch(err => console.error('[svc-talk] turn failed:', err.message))
    }
    ...
    async _reactTurn(bossContent, targetDivisionId) {
        ...
        // hứa "chỉ phòng đó trả lời" — production tự chạy + hiện job card TRÔNG Y NHƯ nó vừa trả lời
        // thay, còn phòng được nhắn thì rơi vào 'waiting_product' (ẩn hoàn toàn khỏi _comVisibleLog)
        // và bị khoá câm luôn ở MỌI lượt sau (_hasWaitingJob) cho tới khi sếp lỡ duyệt đúng job
        // production ẩn danh đó — 2 bug tưởng khác nhau ("production trả lời thay"/"nhắn xong im
        // luôn") thực ra cùng 1 gốc. Chỉ áp dụng khi có target — lượt broadcast (targetDivisionId
        ...
        for (const division of this._sortProductionFirst(divisionsThisTurn, d => d.id)) {
            ...
            try {
                ...
                console.error(`[svc-talk] chatter failed for ${division.id}:`, err.message)
                decision = null
            }
            this._typingId = ''
            ...
            // này ở đây (không đợi tới lượt dispatch job bên dưới) để BỎ LUÔN câu chat xác nhận, dành
            // lời "bắt đầu làm" cho đúng lúc thật sự được đánh thức (_wakeWaitingJobs's
            // `resumeAfterProduct`) — khớp đúng phản hồi: nói "làm ngay" rồi lại phải chờ mới là dư/sai.
            let existingProduct
        ...
        // 1 slot duy nhất, KHÔNG map theo topic — mỗi division tự chạy 1 lượt decideChatterReply
        // RIÊNG, nên topic tự rút ra cho CÙNG 1 yêu cầu của sếp có thể khác chữ giữa các phòng (vd
        // production rút ra "Hộp quà cá nhân hoá" còn marketing rút ra "hộp quà tặng cá nhân hoá") —
        // so khớp topic (chuẩn hoá) từng bị lệch chữ 1 cái là tạo trùng 2 job production cho cùng 1
        // yêu cầu. 1 lượt chat chỉ xoay quanh 1 sản phẩm (giả định MVP, khớp _resolveProduct's exact-
        // match) nên bất kỳ job production nào tạo trong lượt này ĐỀU dùng lại cho mọi division khác
        // cần sản phẩm, bất kể topic chữ có khớp nhau không.
    ...
    // `products` chưa — MVP-level exact match, chưa fuzzy search.
    async _resolveProduct(topic) {
        try {
            ...
            console.error('[svc-talk] failed to resolve product:', err.message)
            return null
        }
    }
    ...
    // webs/report/tools/finance-data.js) — dùng làm giá trị field `financialContext` cho division
    // 'finance' (xem _runJob), TƯƠNG TỰ _comProductContext() ở trên nhưng cho division không neo
    // vào 1 sản phẩm nào. Load lỗi -> trả 1 câu báo THIẾU dữ liệu thay vì để AI tự bịa số
    // (hook/finance_accounting.md §2 "Không bịa số liệu"), buộc câu trả lời rơi vào NEED MORE DATA.
    async _comFinanceContext() {
        try {
            const invoices = await createService('invoices', '', 'invoices').findAll()
            ...
            console.error('[svc-talk] failed to build finance context:', err.message)
    ...
    async _wakeWaitingJobs(productionJobId) {
        ...
        for (const w of waiters) {
            ...
            // post thêm 1 tin nhắn 'chat' riêng nữa (từng làm vậy, ra 2 bubble trùng lặp: 1 job-card
            // có nút Xem chi tiết + 1 chat thường không nút gì — sếp muốn gộp lại còn đúng 1 bubble
            // vừa có text "bắt đầu làm" vừa có nút Xem chi tiết).
    ...
    // từ đầu, dùng cho nút "Tạo lại") — persist Firestore + patch `_log` sau MỖI step. `fromIndex`
    ...
    async _runJob(jobId, division, fromIndex) {
        ...
        // field NGỮ CẢNH (không phải output, chỉ đọc qua contextKeys — xem seed-marketing.js's step
        // 'analysis') tóm tắt tên/mô tả/nội dung sản phẩm thật, để step phân tích đầu tiên "hiểu sản
        // phẩm" và viết đúng thay vì đoán chung chung chỉ từ chuỗi topic ngắn. Sản phẩm có thể vừa
        // được tạo SAU khi job này đã tạo (job từng ở trạng thái 'waiting_product'), nên phải resolve
        // lại ở ĐÂY (lúc pipeline thật sự chạy), không phải lúc tạo job doc.
        if (division.meta?.requiresProduct) {
            ...
            // thu/công nợ/rủi ro tính lại từ `invoices`, xem _comFinanceContext) thay cho
            // productContext ở nhánh trên, cùng lý do: tránh AI tự bịa số (hook/finance_accounting.md §2).
        ...
        for (let i = startIdx; i < steps.length; i++) {
            ...
            try {
                ...
                if (step.type === 'image' && !doc.imageChoice && !patch.pics) {
                    doc = { ...doc, fields, status: 'awaiting_review', pausedStepId: step.id, updated_at: now }
                    await svc.update(jobId, { fields, status: doc.status, pausedStepId: step.id, updated_at: now })
                    this._log = this._log.map(d => d.id === jobId ? doc : d)
                    return
                }
    ...
    // Đánh dấu 1 call/sub-step đang chạy thật (spinner ở <svc-progress>, popup preview) — luôn dọn
    // sạch (finally) kể cả lỗi, tránh spinner treo mãi khi call ném lỗi.
    ...
    // thị nội dung sớm nhất có thể thay vì phải chờ hết step, khớp hành vi onPatch của
    // svc-marketing.js. Chỉ cập nhật state cục bộ `_log` (feedback UI tức thời) — bản Firestore
    // chính thức vẫn chỉ ghi 1 lần ở cuối step trong `_runJob`, không ghi rời từng call.
    ...
    async _dfApprove(jobId) {
        ...
        try {
            ...
            await createService('talks').update(jobId, { approved: true, createdPostId: created.id, updated_at: now })
            this._log = this._log.map(d => d.id === jobId ? { ...d, approved: true, createdPostId: created.id } : d)
            toastEmit(this._txt.approveOk, 'success')
            // Record vừa duyệt (output.table 'products'/'posts') — đồng bộ NGAY vào `mind` để
            // <svc-sale> có dữ liệu tư vấn khách ngay, không cần đợi admin sửa giá/tồn kho trước
            // (xem tools/mind-sync.js — best-effort, tự nuốt lỗi, tự no-op nếu table khác).
            syncMindFromOutputTable(created, output.table)
            // Mirror job đã duyệt vào collection tiến trình của popup admin (division.meta.processTable,
            // vd 'marketing'/'productions'), KEYED THEO created.id — nếu không, admin mở lại đúng
            // record vừa tạo qua nút "Marketing"/"Sản phẩm" trên /admin sẽ thấy popup rỗng (job chat
            // chỉ tồn tại trong `talks`, không có gì ở đó khớp record_id thật vừa tạo). Best-effort —
            // lỗi ở bước này không được làm hỏng việc đã tạo record thành công.
            if (division.meta.processTable) {
                ...
                }).catch(err => console.error('[svc-talk] failed to mirror job into processTable:', err.message))
            }
            ...
            <web-popover ui=${this.ui} theme=${this.theme} placement="top-end" placementGap="16" manualClose>
                ...
                <div class="tlk-panel">
                    <div class="tlk-head">
                        ...
                        <button class="tlk-close" @click=${this._dhClose}>
    ...
    // Popup "Xem chi tiết" — <svc-progress editable> DÙNG CHUNG với svc-marketing.js (cây
    // macro/sub-step + sửa field trung gian inline), mở từ nút Preview trong 1 job bubble bất kỳ
    // (đang xử lý/lỗi/đã xong) — không cần rời khỏi khung chat để xem/chỉnh toàn bộ tiến trình.
    _rfPreviewDialog() {
    ...
    // riêng cho các field này, khớp hành vi svc-marketing.js's `_dhProgressFieldChange`).
    async _dhProgressFieldChange(jobId, e) {
        const { key, value } = e.detail
        this._patchJobFields(jobId, { [key]: value })
        const doc = this._log.find(d => d.id === jobId)
        const now = await createService('talks').now()
        await createService('talks').update(jobId, { fields: doc.fields, updated_at: now })
    }

    // `photo-upload` từ <svc-progress> (sub-step 'upload', job đã done) — user tự upload ảnh từ
    // máy tính để THAY ảnh AI đã tạo, y hệt svc-marketing.js's `_dhPhotoUpload`.
    async _dhPhotoUpload(jobId, url) {
        ...
        const now = await createService('talks').now()
        await createService('talks').update(jobId, { fields: { ...doc.fields, ...patch }, updated_at: now })
    }

    // `resume-step` từ <svc-progress> — sếp bấm "Duyệt" sau 1 step vừa xong (MỌI step `type:'text'`
    // trừ step cuối đều dừng lại chờ, xem _runJob) — chạy tiếp từ ĐÚNG step KẾ TIẾP step vừa bị
    // khoá (không lùi lại chạy lại chính step đó, vì nó đã xong thật).
    _dhResumeStep(jobId, division) {
        if (this._pendingJobs.has(jobId)) return
        const doc = this._log.find(d => d.id === jobId)
        if (!doc || doc.status !== 'awaiting_review') return
        const steps = division.meta.steps
        const pausedIdx = steps.findIndex(s => s.id === doc.pausedStepId)
        // Xoá NGAY `pausedStepId` cục bộ trước khi chạy tiếp — Firestore/doc.status thật chỉ cập
        // nhật SAU KHI step kế chạy xong, nếu không banner "chờ duyệt" + tab step cũ sẽ còn hiện
        // nguyên suốt lúc step kế đang xử lý. Patch cục bộ này để <svc-progress> tự chuyển sang
        // xem step kế (qua autoRunningStep) NGAY khi bấm, không cần đợi.
    ...
    // `regenerate-call` từ <svc-progress> — sếp muốn AI viết lại RIÊNG 1 sub-step (call) kèm chỉ
    // dẫn thêm vừa gõ — granular hơn hẳn regenerate-step cũ (đã bỏ): chạy được bất kỳ lúc nào call
    // đó đã có dữ liệu, không cần job đang paused đúng step chứa nó, và KHÔNG đụng status/pausedStepId
    ...
    _dhRegenerateCall(jobId, division, stepId, callKey, instruction) {
        this._withJobPending(jobId, async () => {
            ...
            const now = await createService('talks').now()
            await createService('talks').update(jobId, { fields, updated_at: now })
            this._log = this._log.map(d => d.id === jobId ? { ...d, fields } : d)
        })
    }

    // `image-choice` từ <svc-progress> — sếp chọn 'ai' (gọi AI sinh ảnh thật) hay 'skip' (bỏ qua,
    // tự làm qua 3 nút mở trang ngoài) tại đúng step ảnh đang bị khoá — lưu lựa chọn rồi chạy LẠI
    // đúng step đó (không phải step kế), vì step ảnh mới chỉ dịch xong concept, chưa thật sự xong.
    // Bọc NGUYÊN thân hàm (kể cả 2 await đầu) qua `_withJobPending` — nếu chỉ guard trước khi gọi
    // `_runJob` thì vẫn có khe hở bấm 2 lần trong lúc đang chờ Firestore ghi `imageChoice`.
    ...
    // `regenerate-image` từ <svc-progress> — tái dùng concept ảnh đã dịch trước đó (hasData luôn
    // true), gọi qua `_enqueue` ở call site như mọi lệnh gọi AI khác (hàng đợi tuần tự toàn cục).
    // `_regeneratingImage` tự guard double-click riêng của nút này; cộng thêm vào `_pendingJobs` để
    // CÁC nút khác của CÙNG job (bubble/preview) cũng disable trong lúc chờ.
    async _dhRegenerateImage(jobId, division) {
        ...
        try {
            ...
            const updated = this._log.find(d => d.id === jobId)
            const now = await createService('talks').now()
            await createService('talks').update(jobId, { fields: updated.fields, updated_at: now })
        } catch (err) {
    ...
    _rfJobContent(m) {
        ...
        if (m.status !== 'done') {
            ...
            // đầu làm" thay vì text "đang xử lý" mặc định, NGAY TRÊN bubble này (không phải 1 bubble
            // 'chat' riêng). KHÔNG hiện nút Thử lại ở đây — tensor.js đã có idle-timeout nên 1 call AI
            // treo/mất kết nối cuối cùng vẫn reject thật, rơi vào nhánh `m.error` phía trên (nút Thử
            // lại CHỈ nên xuất hiện khi có lỗi thật xảy ra).
            return html`
