// src/webs/division/tools/seed-production.js
//
// Chuyên môn phòng "Sản phẩm & Vận hành" bóc tách thành JSON config đọc bởi tools/engine.js — nén
// từ khung 28-step trong hook/production.md (Topic->Customer->JTBD->Problem->PRD->Design->MVP->
// Process->SOP->Quality Standard->QA->QC->Defect->Risk->...->Release) xuống 4 step thực dụng,
// cùng độ chi tiết với tools/seed-marketing.js. Giá bán/SKU/tồn kho CỐ Ý không nằm trong output —
// đó là quyết định kinh doanh thật, không phải thứ AI nên tự bịa (đúng tinh thần production.md's
// "Do not invent facts that require external evidence").
//
// MỌI step `type:'text'` (trừ step cuối) đều tự dừng lại chờ sếp bấm Duyệt trước khi chạy step kế
// — đây là hành vi CHUNG của engine (xem svc-talk.js/admin-base.js's _runJob/_dfRunPipeline, không
// phải config riêng của division này) — tránh AI viết content/tạo ảnh dựa trên 1 kết quả step
// trước sai/chưa ưng mà sếp chưa kịp xem qua.
//
// Division này còn giữ vai trò ĐẶC BIỆT: các division khác có `meta.requiresProduct: true` (vd
// marketing) cần 1 sản phẩm trong bảng `products` khớp topic tồn tại TRƯỚC khi được phép chạy —
// nếu chưa có, svc-talk.js tự dispatch topic đó cho production chạy trước (xem svc-talk.js's
// _reactTurn/_wakeWaitingJobs). Seed 1 lần vào Firestore collection `divisions` (doc id
// 'production') nếu chưa tồn tại — sau đó sửa tiếp qua Xuất/Nhập CSV của /admin/divisions.
export const PRODUCTION_DIVISION_SEED = {
    ...
    meta: {
        ...
        // Collection lưu tiến trình của svc-production.js's admin popup (PHẢI khớp svc-production.js's
        // constructor `this.processTable = 'productions'`) — xem seed-marketing.js's `processTable` cho ý
        // nghĩa đầy đủ (svc-talk.js đọc giá trị này để mirror job chat đã duyệt vào đây).
        processTable: 'productions',
        steps: [
            {
            ...
            },
            {
                ...
                // KHÔNG đổi thứ tự phụ thuộc gì so với lúc còn 2 step riêng, chỉ gộp UI + chạy nhanh
                // hơn (process/titleDesc song song thay vì tuần tự 2 step).
                ...
                calls: [
                    ...
                    },
                    {
                        ...
                        // field `tags` phẳng). Định dạng PHẢI khớp schema thật (products.tags: `multi:true`
                        // — web-texts.js nối/tách bằng '|', KHÔNG phải ','), không thì hiển thị sai ở
                        // /admin/products.
...
Output ONLY the final prioritized keyword list in the field below — never your reasoning, the keyword categories, or any priority scores.`,
        ...
        // 'products' — collection thật của /admin/products (src/services/schemas/admin/products.js).
        // Giá/SKU/tồn kho không nằm trong output — để trống, người dùng tự điền sau trong admin.
