...
> trực tiếp vào schema (`hook/SCHEMA.rst`) và kiến trúc Conductor / Web Boxs / Micro Services
> (`hook/ARCHITECT.rst`) hiện có — **không tạo bảng mới trừ khi không thể tái dùng `records` /
> `users` / `invoice` / `hubs`**.
...
- **`hook/SCHEMA.rst` mô tả field vocabulary dùng chung** (title/description/content/tags/pics/
  ...
  **collection riêng** (`talents`/`jobs`/`proposals`/`walletTxns`) nhưng **dùng chung field
  vocabulary** của `records` trong `hook/SCHEMA.rst` — mọi chỗ dưới đây ghi `mode: 'talent'` nghĩa
...
- **Tái dùng field thật trước, chỉ rơi xuống `meta.xxx` khi không có field phù hợp** — đúng quy tắc
  đã áp dụng cho mọi loại dữ liệu khác trong `hook/SCHEMA.rst`.
- **Không phình bảng mới**: Proposal + Negotiation + Deal gộp làm **một** record (`mode:
  ...
  hàng (`hook/PAY.rst`) — thay vì tạo record mới ở mỗi bước.
...
- Domain mới là **leaf domain** `src/webs/talent/`, không import ngược từ `bay`/`pay`/`socials`;
  kết nối bằng props khi mount (giống nguyên tắc isolation của `webs/pay` trong `hook/PAY.rst`).
...
| Escrow thanh toán Deal (VNĐ thật) | `invoices` (server `'DB_ALL'` mặc định, xem `_invoiceSvc()` trong `webs/pay`) | — | tái dùng nguyên collection, `order_id` → `proposals.id` |
...
| `tags` | Skills, pipe-separated: `'nodejs\|python\|postgresql\|aws'` — dùng luôn cơ chế filter theo `tags` đã có (`sift`, xem `hook/SERVICES.rst`) |
...
| `score` | `avg~count` tổng — parse/update dùng đúng công thức trong `hook/SCHEMA.rst` §`score format` |
...
đã mô tả cho `meta.views`/`meta.likes` trong `hook/SCHEMA.rst` (`bumpMeta`,
...
`invoice.meta` (`hook/PAY.rst`), áp lại cho negotiation thay vì shipping.

### 1.5 `mode: 'review'`

Mode này **đã được khai báo trong convention table của `hook/SCHEMA.rst`** nhưng chưa có
...
- Thêm token `talent` và/hoặc `employer` vào `users.roles` (pipe-separated, cộng thêm bên cạnh
  ...
  capability RBAC** (phân biệt rõ ở §6.1, tránh nhầm với `hook/AUTH_ROLES.rst`).
...
- **Escrow "giữ tiền" và "release"** không phải trạng thái hợp pháp của `invoice.status` (invoice
  là văn bản pháp lý bất biến sau `issued`, theo đúng nguyên tắc trong `hook/SCHEMA.rst` §"Điểm
  ...
  `hook/PAY.rst` đã áp dụng cho `invoice.meta.major/sub`.
...
`sift()`-side filter theo `tags` (client-side, xem `hook/SERVICES.rst` §`sift`) — không cần query
...
Áp lại chính xác mẫu `major/sub` đã dùng cho đơn hàng trong `hook/PAY.rst`
...
- Mỗi lần đổi `stage`/thêm `history[]` entry: **read-modify-write** `findById` → merge `meta` →
  `update` (JSONB không tự deep-merge, đúng cảnh báo trong `hook/CRUD.rst`).
- Ai thao tác được ghi lại theo kiểu tilde-encode `"<ts>~<userId>~<name>~<action>"` — cùng kỹ thuật
  `parseHandler()` mà `webs/pay` dùng để lưu "ai xác nhận bước nào" (`hook/PAY.rst`).
...
- **Cache hiệu năng**: `users.meta.xuBalance` — cộng/trừ ngay khi tạo `wallet_txn` mới, theo đúng
  ...
  cách `hook/SCHEMA.rst` đã chấp nhận cho `meta.views`/`meta.likes` (phù hợp traffic marketplay
...
- Nếu về sau cần audit chặt (đối soát nạp Xu bằng tiền thật ở scale lớn), migrate sang server
  Firestore riêng qua `registerAdapter` (`hook/CRUD.rst`) — không đổi shape `wallet_txn`.
...
- Action ghi vào `talent.actors` với action mới `'verified'` (mở rộng enum actors ngoài 7 giá trị
  có sẵn trong `hook/SCHEMA.rst`, tương thích ngược vì `actors` chỉ parse tự do theo `~`).
...
- Set `talent.meta.verification.professional = true` (+ optional `meta.verification.verifiedBy`,
  ...
  lý do), tái dùng đúng schema của `revisions` (`hook/AUTH_DIFFS.rst`) thay vì thiết kế bảng mới.
...
Employer trả 100 Xu (unlock_contact, §3.3) hoặc escrow đã issued
   ↓
talent.secure += ',<employerId>~read'     // ACL entry format đúng hook/SCHEMA.rst
...
sẵn để hook vào — cân nhắc tái dùng `hook/CHATS.rst` P2P chat khi triển khai).
...
- **`users.roles` hiện có** (`user`/`editor`/`admin`) — capability RBAC toàn hệ thống, dùng bởi
  `auth.isAdmin()`/`auth.hasRole()` (`hook/ARCHITECT.rst` §Auth Service).
...
Theo đúng cấu trúc leaf-domain của `hook/ARCHITECT.rst` §Micro Service Pattern, tham chiếu trực
...
đã chuẩn hoá trong `hook/ARCHITECT.rst` §Comment Convention):
...
| `talentProfileHero` | mẫu `M1` (text 7 + pinned image 5) trong `hook/DESIGN.rst` | Header profile (avatar, badges, rate, availability) |
...
(`web-currency`, cho `pricing`) — tất cả đã có sẵn trong `hook/web-apex.rst`, không cần component
mới.
...
1. **Schema**: thêm `mode: 'talent' | 'job' | 'proposal' | 'wallet_txn'` vào tài liệu convention
   (`hook/SCHEMA.rst` §"mode convention") — cập nhật doc trước khi code theo đúng thói quen của repo.
...
3. **`src/webs/talent/tools/service.js`** — CRUD wrapper qua `createService('talents')` /
   ...
   `hook/CRUD.rst`, mỗi collection riêng — xem §1.1) + toàn bộ hàm nghiệp vụ liệt kê ở §7.
...
11. **Escrow**: hàm `issueEscrowInvoice` tái dùng đúng `createService('invoices')`
    (giống `_invoiceSvc()` trong `src/webs/pay/tools/service.js`) — không viết lại adapter mới.
12. **RBAC**: cập nhật `hook/AUTH_ROLES.rst` + capability map trong `src/webs/auth/svc-roles.js`
...
Repeat hire / Favorite Talent (dùng `users.connections`-style list, xem `hook/SCHEMA.rst`) · Saved
