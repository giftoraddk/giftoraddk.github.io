// src/webs/llm/tools/seed-marketing.js
//
// Chuyên môn phòng Marketing bóc tách thành JSON config đọc bởi tools/engine.js. Cloned from
// src/webs/division/tools/seed-marketing.js (domain-isolation clone) — `steps`/`output` unchanged.
//
// `processTable: ''` (khác bản division gốc, vốn là 'marketing') — domain llm KHÔNG có admin popup
// riêng ("AI Content" gắn vào /admin/posts) để mirror job đã duyệt vào, khác svc-marketing.js bên
...
Output ONLY the final prioritized keyword list in the field below — never your reasoning, the keyword categories, or any priority scores.`,
        ...
        // collection riêng, xem src/pages/post/index.astro's fetchCollection('posts')) — bảng này
        // GIỮ NGUYÊN Firestore DB_ALL, không chuyển sang D1 (xem svc-talk.js's _dfApprove).
