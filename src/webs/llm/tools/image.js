// src/webs/llm/tools/image.js
//
// Image-generation pipeline shared by any division step with `type: 'image'` (currently only
// `marketing`/`production` in this domain). Cloned from src/webs/division/tools/image.js —
// domain-isolation clone, logic unchanged. AI image gen goes through worker's /v1/ai/image
// (via services/api.js's llmFetch) — worker now owns ALL AI processing app-wide, not just
// this domain's D1 data (see worker/packages/llm-worker/src/ai.ts's header).
 ...
 * gọi Hugging Face. Tách riêng khỏi runImageGenerate() để caller (svc-talk.js) có thể dừng pipeline
 * NGAY sau bước này, chờ sếp chọn "Tạo ảnh bằng AI" hay "Bỏ qua" trước khi tốn 1 lệnh gọi ảnh thật
 * — xem svc-progress.js's image-choice UI.
 *
 * @param {object}   opts
...
export async function runImageGenerate({ topic, fields, colors, runSub }) {
    ...
    // Hugging Face key + cold-start retry (HF free serverless models occasionally 503 "model
    // loading" on the first call) live server-side in worker's Worker (worker/packages/llm-worker/src/ai.ts's
    // handleAiImage) — this call throws only after the Worker's own retries are exhausted.
    const blob = await runSub('generate', async () => {
        const res = await llmFetch('/v1/ai/image', {
            method: 'POST',
            body: { prompt: promptText, negativePrompt: IMAGE_NEGATIVE_PROMPT },
        })
        return res.blob()
    })
