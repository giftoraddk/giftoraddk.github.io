// src/webs/division/tools/admin-base.js
//
// DivisionAdminBase — orchestrator LitElement dùng chung bởi svc-marketing.js VÀ svc-production.js
// (mỗi phòng ban 1 popup AI content riêng, attach vào 1 hàng web-table qua prop `marketing`/
// `production` — xem web-table.js/svc-admin.js). KHÔNG tự đăng ký customElements — mỗi subclass tự
// `customElements.define(...)` tag riêng + khai default props/SEED/TXT_STD/SAVED_EVENT của mình.
//
// Progress persisted vào Firestore collection `processTable`, 1 doc / record_id, overwritten khi
// regenerate — failed step retry không mất lại từ đầu. UI tách hẳn ra 2 component con nói chuyện
// qua event: <svc-progress> (cây macro/sub-step, dùng chung với svc-talk.js) và <svc-review> (form
// output cuối title/description/content/pics) — xem hook/superpowers/specs/2026-09-05-division-
// talk-design.md.
...
import '@/webs/division/svc-review.js'
...
export class DivisionAdminBase extends LitElement {
    ...
    static properties = {
        table:    { type: String },  // bảng đích ghi kết quả cuối (division.meta.output.table)
        processTable: { type: String },  // collection lưu tiến trình
        division: { type: String },  // doc id trong collection `divisions`
        ai:       { type: String },
        ...
        _division:    { state: true },  // doc từ collection `divisions` (config chuyên môn)
        _doc:         { state: true },
    ...
    async _dcLoadDivision() {
        ...
        try {
            const doc = await createService('divisions').findById(this.division)
            if (doc) { this._division = doc; return }
        } catch (err) {
            console.error(`[${Ctor.name}] failed to load division config:`, err.message)
        }
        // Chưa có trong Firestore (lần đầu chạy) — dùng seed local ngay, rồi ghi vào DB để lần sau
        // đọc thẳng + sửa được qua Xuất/Nhập CSV của /admin/divisions.
    ...
    async _dcLoadDoc(recordId) {
        this._runningStep = 0
        this._failedStep  = 0
        // <svc-marketing>/<svc-production> là 1 instance dùng lại cho nhiều row (svc-admin.js gọi
        // open(id) mỗi lần) — xoá sạch tiến trình con của lượt trước, tránh hiện nhầm dấu tick của
        // record A khi vừa mở record B.
        this._runningSub = new Set()
        this._regeneratingImage = false
        try {
    ...
    // `field-change` từ <svc-progress> — field TRUNG GIAN sửa xong là persist NGAY vào processTable's
    // doc — không có nút "Lưu" riêng cho các field này.
    ...
    // `photo-upload` từ <svc-progress> (sub-step 'upload', review phase) — user tự upload ảnh từ
    // máy tính để THAY ảnh AI đã tạo. Thay hẳn <figure> ảnh cũ trong content bằng ảnh mới (hoặc bỏ
    // hẳn nếu url rỗng), cùng cách _dfRegenerateImage() làm, rồi persist ngay vào doc mkt (KHÔNG
    // đụng tới bản ghi cuối cùng ở `table`, chỉ ghi khi Lưu).
    ...
    // trừ step cuối đều dừng lại chờ, xem _dfRunPipeline) — chạy tiếp từ ĐÚNG step KẾ TIẾP step vừa
    // bị khoá.
    _dfResumeStep() {
    ...
    // đó đã có dữ liệu, không cần doc đang paused đúng step chứa nó, và KHÔNG đụng status/
    ...
    // thay vì JSON). PUBLIC_NVID~pinned~<model> ép model cụ thể qua trước (nvidia/nemotron-3.5-
    // lightning-30b-a3b — general-purpose, KHÔNG phải reasoning model), rồi mới rơi xuống this.ai
    // làm lưới an toàn nếu model ghim lỗi/hết quota.
    get _comJsonAi() {
