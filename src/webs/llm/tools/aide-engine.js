...
import { toPrice } from '@/services/helper.js'

const LANG_NAMES = { vi: 'Vietnamese', en: 'English' }

// `meta.pricing` is stored "price~cost~unit" (see toPrice()/schemas/admin/products.js) — `cost` is
// internal COGS and must NEVER reach the prompt (nothing stops the model from just repeating
// whatever it's given). Only the sell price + unit are shown to the AI.
function _formatPrice(pricing) {
    const price = toPrice(pricing)
    if (!price) return '(not set)'
    const unit = String(pricing || '').split('~')[2]?.trim()
...
Price rule: quote each PRICE EXACTLY as written above — never invent, round, or average a range.
If PRICE is "(not set)", say the Vietnamese/English equivalent of "Giá: Liên hệ" ("Price: contact
us") for that item instead of guessing a number.
